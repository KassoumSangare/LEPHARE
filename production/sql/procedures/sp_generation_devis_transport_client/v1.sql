...
BEGIN
	...
	LOOP 
		...
		SELECT SUM(primenette), SUM(accessoire), SUM(taxe), SUM(primettc), COUNT(DISTINCT assure), SUM(CASE WHEN accessoire < 500 THEN 0 ELSE 500 END)
		...
		IF (id_devis = 0) THEN --Pas de dévis existant pour cette période pour cette police du client
			...
			VALUES (user_id, id_offre, id_intermediaire, id_compagnie, id_produit, id_client, id_assure, id_avenant, numero_avenant, numero_devis, flotte_transport, id_compagnie, 
					...
					montant_accessoire - montant_accessoire_client, montant_taxe, prime_ttc) RETURNING IdDevis INTO id_devis;
			...
			INSERT INTO public.StdDevisDetail(IdDevis, IdOffre, IdTarif, Vehicule, idusage, CodeUsage, Reference, IdCarrosserie, PuissanceFiscale, NombrePlace,
					...
					DateMutation, NumMoteur, NumChassis, TypeVehicule, IdMarque, Matricule, CarteVerte, Essence, NumPCCnd, TauxReduction, primenette, taxeenregistrement)
		...
		ELSE
			...
			SET IdOperateur = user_id, IdOffre = id_offre, IdIntermediaire = id_intermediaire, IdCompagnie = id_compagnie, IdProduit = id_produit, IdClient = id_client, IdAssure = id_assure, IdAvenant = id_avenant, Flotte = flotte_transport,
				...
				NumeroPoliceConnexe = numero_police_guce, PrimeNette = prime_nette, AccessoireIntermediaire = montant_accessoire - montant_accessoire_client, AccessoireCompagnie = 0, Accessoire = montant_accessoire - montant_accessoire_client, Taxe = montant_taxe, PrimeTTC = prime_ttc
