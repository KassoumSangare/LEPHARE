...
BEGIN
		...
		ELSIF (code_avenant IN ('REN', 'INC')) THEN
			...
			IF (code_avenant = 'REN') THEN
				...
				LOOP
					...
					IF id_produit = 5 THEN --Production Santé
						...
						SELECT PrimeFamille, PrimeAffilie, PrimeGlobale, MontantSurprime, MontantAccessoireManuel, TauxReductionCommerciale, IdTypeContrat, GestionnaireSante, id_devis_detail
			...
			-- Santé MINENE : copier StdDevisDetail, garanties, complément, adhérents et affiliés
			IF id_produit = 5 THEN
				-- Ne copier StdDevisDetail/garanties/complément que si la boucle auto ne l'a pas déjà créé
				IF NOT EXISTS (SELECT 1 FROM StdDevisDetail WHERE IdDevis = id_devis) THEN
					SELECT IdDevisDetail
					INTO id_devis_detail_ancien
					FROM StdDevisDetail
					WHERE IdDevis = id_devis_ancien
					ORDER BY IdDevisDetail
					LIMIT 1;

				IF id_devis_detail_ancien IS NOT NULL THEN
					INSERT INTO public.stddevisdetail(iddevis, idtarif, vehicule, codeusage, reference, idcarrosserie, puissancefiscale, nombreplace, chargeutile, valeurneuve, valeurvenale, valeuraccessoire, remorque, matrem1, matrem2, extincteur, idprofession, conducteur, adressecnd, villecnd, sexe, datemec, datemutation, nummoteur, numchassis, nbreextinteur, typevehicule, idmarque, matricule, typeimmat, attestation, provisoire, permis, datedelipc, datenaicnd, iddelegation, agrement, villeagrement, observation, idoldhist, oldpolice, oldattestation, oldmarque, oldtype, oldmatricule, carteverte, numcarteverte, primeannuelle, primenette, mt_delegation, essence, attestationprov, cg_control, numpccnd, pctype, idoffre, taxeenregistrement, pc_control, idgenrevehicule, idtypevehicule, modelevehicule, idusage, codealarme, auteur, fga, tauxreduction, primeimposee)
					SELECT id_devis, idtarif, vehicule, codeusage, reference, idcarrosserie, puissancefiscale, nombreplace, chargeutile, valeurneuve, valeurvenale, valeuraccessoire, remorque, matrem1, matrem2, extincteur, idprofession, conducteur, adressecnd, villecnd, sexe, datemec, datemutation, nummoteur, numchassis, nbreextinteur, typevehicule, idmarque, matricule, typeimmat, '' AS attestation, provisoire, permis, datedelipc, datenaicnd, iddelegation, agrement, villeagrement, observation, id_devis_ancien, numero_police, attestation, idmarque, oldtype, matricule, carteverte, '' AS numcarteverte, 0, 0, mt_delegation, essence, attestationprov, cg_control, numpccnd, pctype, idoffre, 0, pc_control, idgenrevehicule, idtypevehicule, modelevehicule, idusage, codealarme, auteur, 0 AS fga, tauxreduction, primeimposee
					FROM public.stddevisdetail
					WHERE IdDevis = id_devis_ancien AND IdDevisDetail = id_devis_detail_ancien
					RETURNING IdDevisDetail INTO id_devis_detail;
					...
					WHERE IdDevisDetail = id_devis_detail_ancien;
				END IF;
				END IF; -- fin NOT EXISTS StdDevisDetail

				-- Copier les adhérents et affiliés avec remappage des IDs
				FOR var_adherent IN (SELECT * FROM StdAdherent WHERE iddevis = id_devis_ancien) LOOP
					INSERT INTO StdAdherent(nom, prenom, sexe, vip, mobile1, mobile2, email, dateadhesion, datesortie, actif, iddevis, datedebutconsommation, idfiliale, idoperateur, cni, adresse, groupesanguin, matricule, montantsurprime, surprimeappliquee, datenaissance, fichierpiece, nombrepathologie, numerocmu)
					VALUES (var_adherent.nom, var_adherent.prenom, var_adherent.sexe, var_adherent.vip, var_adherent.mobile1, var_adherent.mobile2, var_adherent.email, var_adherent.dateadhesion, var_adherent.datesortie, var_adherent.actif, id_devis, var_adherent.datedebutconsommation, var_adherent.idfiliale, user_id, var_adherent.cni, var_adherent.adresse, var_adherent.groupesanguin, var_adherent.matricule, var_adherent.montantsurprime, var_adherent.surprimeappliquee, var_adherent.datenaissance, var_adherent.fichierpiece, var_adherent.nombrepathologie, var_adherent.numerocmu)
					RETURNING idadherent INTO v_new_adherent_id;

					INSERT INTO StdAffilie(lien, nom, prenom, datenaissance, dateadhesion, datesortie, certificat, datectrl, matricule, carte_vigueur, ancien_matricule, date_dernier_demande, handicape, sexe, actif, primeannuelle, iddevis, datedebutconsommation, idadherent, idoperateur, observations, cni, montantsurprime, surprimeappliquee, groupesanguin, mobile1, mobile2, fichierpiece, nombrepathologie, numerocmu)
					SELECT lien, nom, prenom, datenaissance, dateadhesion, datesortie, certificat, datectrl, matricule, carte_vigueur, ancien_matricule, date_dernier_demande, handicape, sexe, actif, primeannuelle, id_devis, datedebutconsommation, v_new_adherent_id, user_id, observations, cni, montantsurprime, surprimeappliquee, groupesanguin, mobile1, mobile2, fichierpiece, nombrepathologie, numerocmu
					FROM StdAffilie
					WHERE iddevis = id_devis_ancien AND idadherent = var_adherent.idadherent;
				END LOOP;

				INSERT INTO StdNumeroSaisieSante(datecreation, datemaj, saisieencours, iddevis, idoperateur)
				VALUES (CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, TRUE, id_devis, user_id);
			END IF;
