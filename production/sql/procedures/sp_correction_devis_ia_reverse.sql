...
BEGIN
		...
		LOOP
			...
			SET primeannuelle = CASE WHEN IdGarantie = 17 THEN ROUND (p_prime_annuelle * 7 / (20 * v_nombre_assures), 0)
								  WHEN IdGarantie = 19 THEN ROUND (p_prime_annuelle * 5 / (5 * v_nombre_assures), 0)
								  WHEN IdGarantie = 20 THEN ROUND (p_prime_annuelle / (4 * v_nombre_assures), 0)
							 ELSE primeannuelle
							 END,
				primenette = CASE WHEN IdGarantie = 17 THEN ROUND (p_prime_nette * 7 / (20 * v_nombre_assures), 0)
								  WHEN IdGarantie = 19 THEN ROUND (p_prime_nette * 5 / (5 * v_nombre_assures), 0)
								  WHEN IdGarantie = 20 THEN ROUND (p_prime_nette / (4 * v_nombre_assures), 0)
							 ELSE primenette
							 END
...
END;
$BODY$;
ALTER PROCEDURE public.sp_correction_devis_ia
    OWNER TO uranususer;
