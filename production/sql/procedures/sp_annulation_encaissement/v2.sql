...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM public.stddetailencaissement WHERE idencaissement = id_encaissement AND montantreglement > 0) THEN
		out_message := 'Encaissement déjà reversé. Annulation impossible.';
		RAISE EXCEPTION '%', out_message;
	END IF;
	...
	INSERT INTO public.StdEncaissement(NumeroPiece, DateEncaissement, MontantEncaissement, MontantEnAttente, MontantDeduit, NumeroCheque, Compte_Compensation,
									   ...
									   IdBanque, IdModePaiement, Created_At, Updated_At)
	SELECT NumeroPiece, date_annulation AS DateEncaissement, -MontantEncaissement, -MontantEnAttente, -MontantDeduit, NumeroCheque, Compte_Compensation,
		...
		date_saisie_annulation AS DateSaisieAnnulation, NomTireurCheque, IdBanque, IdModePaiement, date_saisie_annulation, date_saisie_annulation
	FROM public.StdEncaissement
	WHERE IdEncaissement = id_encaissement
	RETURNING IdEncaissement INTO id_nouvel_encaissement;

	UPDATE public.StdEncaissement
	SET DateAnnulation = date_annulation, DateSaisieAnnulation = date_saisie_annulation, Piece_Annulee = True, NomAnnulation = nom_annulation, MotifAnnulation = motif_annulation, Updated_At = date_saisie_annulation
	WHERE IdEncaissement = id_encaissement;
