...
BEGIN
	...
	v_taux_taxe := public.fn_get_taux_taxe(id_compagnie, id_produit, id_offre, date_effet);
	IF id_offre IN (173, 174) AND prime_nette <> 0 AND prime_ttc <> 0 THEN
		montant_accessoire := ROUND(prime_ttc / (1 + (v_taux_taxe / 100)) - prime_nette, 2);

		UPDATE public.stddevis
		SET Accessoire = COALESCE(Accessoire, 0) + montant_accessoire
		WHERE iddevis = id_devis;
	END IF;
	
	charge_utile := montant_accessoire;
	...
	FROM fn_garantie_offre_ia(id_compagnie, id_offre, capital_deces, capital_ipp, frais_traitement, taux_reduction, date_effet,
				date_expiration, code_activite, date_naissance, prime_nette, ROUND(montant_accessoire, 0))
	WHERE idgarantie <> 0
	ORDER BY 1;
