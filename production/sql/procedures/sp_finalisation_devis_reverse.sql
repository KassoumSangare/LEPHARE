...
-- 			END IF;
		...
		IF id_offre <> 66 THEN --Offres autres que l'offre IA MINENE
			...
			SET primeannuelle = M.primeannuelle, primenette = M.primenette, fga = M.fga, taxe = M.taxeenregistrement, CommissionIntermediaire = ROUND((M.primenette*COALESCE(taux_commission,0))/100,0)
			FROM (SELECT iddevis, SUM(primeannuelle) AS primeannuelle, SUM(primenette) AS primenette, SUM(fga) AS fga, SUM(taxeenregistrement) AS taxeenregistrement
				  FROM StdDevisDetail
				  WHERE iddevis=id_devis
		...
		ELSE
			UPDATE StdDevis
			SET primeannuelle = 0, primenette = 0, fga = 0, taxe = 0, CommissionIntermediaire = 0
