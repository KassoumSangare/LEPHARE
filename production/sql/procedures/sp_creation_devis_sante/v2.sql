...
BEGIN
	...
	ELSIF id_offre IN (17, 18, 19, 20, 21, 22, 175, 176, 181) THEN ---Offres MINENE
		...
		IF (prime_affilie = 0) AND (prime_famille = 0) AND (prime_globale = 0) THEN
			...
			IF taux_reduction_commerciale <> 0 THEN --Réduction ou Majoration commerciale (tous contrats)
				prime_affilie := ROUND(prime_affilie * (1 - (taux_reduction_commerciale / 100)));
			END IF;
