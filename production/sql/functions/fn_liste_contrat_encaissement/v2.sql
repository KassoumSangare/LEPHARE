...
CREATE OR REPLACE FUNCTION public.fn_liste_contrat_encaissement(
    ...
    RETURNS TABLE(idcontrat integer, idclient integer, libellecategorie character varying, numeropolice character varying, numeropoliceinterne character varying, numeroavenant character varying, numeroquittance character varying, nomclient character varying, dateeffet date, dateexpiration date, dateemission date, duree integer, primenette numeric, taxeenregistrement numeric, commission numeric, accessoire numeric, primettc numeric, montantencaisse numeric, solde numeric) 
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	IF (reference_client <> '') AND (reference_contrat <> '') THEN
		RETURN QUERY
			...
			COALESCE(SD.NumeroPoliceCompagnie, SD.NumeroPolice) AS numeropolice, SD.NumeroPolice AS numeropoliceinterne, SD.NumeroAvenant, COALESCE(SQ.NumeroQuittance, '') AS numeroquittance,
